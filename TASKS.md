...
| **T0.3** ✅ | CI pipeline: GitHub Actions for lint, format:check, build, unit tests on PR + push                                                                                                                                                                                                                                      | **DONE** — `.github/workflows/ci.yml` runs lint + format:check + build + test on Node 18/20 for pushes/PRs to `main` and `claude/**`                                                                                                                                                     | Haiku 4.5    |

---

## Phase 1 — API client library (`huckleberry-api` equivalent)

> ⚠️ **Schema-mismatch rework (T1.3, T1.5–T1.9).** A review against
> `py-huckleberry-api` found the models and write ops use an **invented Firestore
> schema**, not the real one: Firestore `Timestamp` objects instead of
> epoch-second floats, fabricated field names (`startTime`/`status`/`date` vs
> `start`/`mode`/`offset`/`lastUpdated`), no timezone `offset`, no parent `prefs`
> summary updates, the wrong interval-id scheme, and 3 wrong collections
> (pump→`pump`, diaper/potty→`diaper`, growth→`health/data`). Unit tests pass only
> because they assert the code against itself. These must be **ported from the
> Python source and verified live** before they can be trusted — the T3.2 harness
> (`npm run inspect:schema`) exists to ground-truth the real shapes. T1.4 (reads)
> and T2.x (tool wiring) are unaffected in structure.
...
| **T1.5** 🟡 | Sleep ops: `start/pause/resume/cancel/complete/log` + history                                                                    | **DONE** — `src/client/sleepOps.ts`; all 6 write ops + `getSleepHistory`, unit-tested                                                                                                                                      | Sonnet 4.6 |
| **T1.6** 🟡 | Feeding ops: nursing `start/pause/resume/switchSide/complete/log`, `logBottle`, `logPump`, `listPumpIntervals`                   | **DONE** — `src/client/feedOps.ts`; all 9 ops + `getFeedHistory`, unit-tested                                                                                                                                              | Sonnet 4.6 |
| **T1.7** 🟡 | Diaper & potty: `logDiaper` (pee/poo/both/dry + color/consistency), `logPotty`                                                   | **DONE** — `src/client/healthOps.ts`; `logDiaper` + `logPotty`, unit-tested                                                                                                                                                | Sonnet 4.6 |
| **T1.8** ⛔ | Growth: `logGrowth` (weight/height/head circ., metric/imperial), `getLatestGrowth`                                               | **DEFERRED / not implemented** — existing `growthOps.ts` uses a wrong schema (`growth/{cid}/records`); real path is `health/{cid}/data` but no live sample captured yet. Revisit once a growth entry is logged + inspected | Sonnet 4.6 |
| **T1.9** 🟡 | Solids: `listCuratedFoods`, `listCustomFoods`, `createCustomFood`, `logSolids`                                                   | **DONE** — `src/client/solidsOps.ts`; Cloud Storage fetch + Firestore custom foods + logSolids, unit-tested                                                                                                                | Sonnet 4.6 |
