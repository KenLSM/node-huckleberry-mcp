...
| ID        | Task                                                                                                                                                                                           | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Model      |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| ~~B1~~ ✅ | ~~Add optional `notes` to the remaining `log_*` tools (`log_nursing`, `log_solids`, `log_pump`, `log_diaper`, `log_potty`, `log_sleep`, `log_growth`)~~                                        | **Done.** `notes?` threaded through every op + tool; read models surface `notes`; unit tests assert it; the live write round-trip verifies `notes` persists end-to-end.                                                                                                                                                                                                                                                                                                                                                                                            | Haiku 4.5  |
| ~~B2~~ ✅ | ~~`edit_*` tools for the other trackers (sleep, diaper, pump, growth) + expose the doc `id` in their history reads~~                                                                           | **Done.** Added `edit_sleep`/`edit_pump`/`edit_diaper`/`edit_growth` (+ ops) on the `edit_feed` pattern; sleep/diaper/pump/growth reads now return the doc `id`. Unit tests + live edit round-trip cover it; 24 tools total.                                                                                                                                                                                                                                                                                                                                       | Haiku 4.5  |
| ~~B3~~ ✅ | ~~`delete_*` (or `cancel_*`) tools to remove a logged entry~~                                                                                                                                  | **Done.** `delete_sleep`/`delete_feed`/`delete_pump`/`delete_diaper`/`delete_growth` (+ ops) `deleteDoc` the entry by id. Decision: do **not** recompute `prefs.last*` on delete (may briefly show a stale "most recent" until the next write — documented). Unit tests assert paths; live round-trip already deletes every tracker. 29 tools total.                                                                                                                                                                                                               | Sonnet 4.6 |
...
| B5 🟡     | **Real solids food tracking** — `log_solids` records `foods` + `reaction`; `create_custom_food` fixed to the real custom-food doc shape                                                        | **Implemented to the Python shape (ported), pending live app confirmation.** `log_solids` writes `foods: {id:{id,created_name,source,amount?}}` + `reactions: {LOVED\|MEH\|HATED\|ALLERGIC:true}`; `create_custom_food` now writes `{id,name,type:"solids",source:"custom",archived,image,created_at,updated_at}` (ISO ts, doc id == id); `list_custom_foods` filters archived + sorts. Unit + gated live round-trip cover it. **TODO (verify-first):** confirm the docs render in the app — field names may differ. `foodNoteImage` (meal photo) not implemented. | Sonnet 4.6 |
| ~~B6~~ ✅ | **API/Firestore discovery harness** — upgrade `inspect-schema` to a configurable multi-path prober (exists/empty/not-found + JSON output) and seed a living `docs/firestore-schema.md` catalog | **Done.** `scripts/inspect-schema.mjs` now probes verified + candidate paths, reports doc✓/✗ + subcollection counts + `mode`/`type` discriminators, takes `PROBE_PATHS`/`ONLY_PROBES`/`SAMPLE`/`OUT`, and writes a gitignored JSON dump. Seeded `docs/firestore-schema.md` (living catalog). Implements `docs/discovery-plan.md`; underpins B5. **Needs a live run on a real account to confirm candidates.**                                                                                                                                                      | Sonnet 4.6 |
