...
| **T1.8** 🟡 | Growth: `logGrowth` (weight/height/head circ., metric/imperial), `getLatestGrowth`                                               | **READY** — live sample captured; real shape is `health/{cid}/data` `{ mode:"growth", start, offset, lastUpdated, weight?/weightUnits, … }` and growth does NOT update `prefs`. Build per `docs/schema-port-spec.md` §3.6 | Haiku 4.5  |
