...
| B5 ⚠️     | **Real solids food tracking** — `log_solids` should record `foods` + `reaction`; fix `create_custom_food` to the real custom-food doc shape                                                    | Gap vs the Python API (which our solids was ported from): `log_solids` stores `foods: {id:{id,created_name,source,amount}}`, `reactions: {LOVED\|MEH\|HATED\|ALLERGIC: true}`, `foodNoteImage`. Our `create_custom_food` writes a **non-conformant** doc (missing `type:"solids"`/`archived`/`source`/`image`, numeric instead of ISO timestamps, invented fields) → likely won't show as solids in the app. **Verify-first** (see `docs/discovery-plan.md`): capture the real docs before changing the write. | Sonnet 4.6 |
